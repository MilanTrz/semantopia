<script lang="ts">
    ...
    let findAnagrams:boolean = false;
	...
	<div class="mx-auto max-w-3xl">
		<div class="mb-6">
			<div class="mb-8">
                ...
                <h2>Mot a déchiffrer : {wordShuffleFind}</h2>
                <h2>Temps restants : {count}</h2>
			</div>
		</div>
		{#if isLoading}
        ...
        {#if isGameOver}
			...
			>
				<p class="text-3xl font-bold text-red-700">
					Partie terminée, vous avez deviné {nbAnagramsFind} annagrammes.
                    Le dernier mot était {wordToFind}.
				</p>
			</div>
		{/if}
