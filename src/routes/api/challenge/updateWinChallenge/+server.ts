...
export async function POST({ request }: RequestEvent) {
	...
	try {
		const [rowsChallenge] = (await pool.query(
			'SELECT COMPLETEDTODAYCHALLENGE FROM STATISTICS WHERE USER_ID = ?',
			[idUser]
		)) as [Array<{ COMPLETEDTODAYCHALLENGE: boolean }>, unknown];
		...
		if (!completedTodayChallenge) {
			await pool.query(
				'UPDATE STATISTICS SET NB_CHALLENGE = NB_CHALLENGE + 1, COMPLETEDTODAYCHALLENGE = 1 WHERE USER_ID = ?',
				[idUser]
			);
